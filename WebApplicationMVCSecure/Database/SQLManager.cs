...
{
    public class SQLManager<TEntity> : DbContext where TEntity : class
    {
        public SQLManager(DataConnectionResource dataConnectionResource) 
            : base(EnumString.GetStringValue(dataConnectionResource))
        {
            SQLFullDB initDBIfNotExist = new SQLFullDB(dataConnectionResource);
        }
